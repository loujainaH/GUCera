...
create proc cInstructors
@cid int
As
IF(EXISTS(SELECT * FROM Course WHERE id = @cid))
Begin
Select C.name, C.id, Users.id,Users.firstName,Users.lastName from InstructorTeachCourse I 
inner join Course C on C.id = I.cid
inner join Users on Users.id = I.insid 
where C.id = @cid
End
go
...
AS
select name, id from Course where accepted ='1'
go
...
WHERE STC.cid = @cid AND STC.sid = @sid AND C.id = @cid AND A.[type] = @assignType and STC.assignmenttype=@assignType ))
...
and A.number=@assignnumber and STC.assignmenttype=@assignType
ELSE
